...
<aside class="flex h-full min-h-0 w-56 shrink-0 flex-col border-r border-white/[0.08] bg-neutral-950/35 p-4 backdrop-blur-xl">
  ...
  {#if contextTags.length}
    <section class="mb-5"><div class="mb-2 text-[12px] font-bold uppercase tracking-wider text-[var(--text-secondary)]">Instance context</div><div class="flex flex-wrap gap-1.5">{#each contextTags as tag (tag)}<span class="inline-flex items-center gap-1 rounded-md border border-emerald-500/25 bg-emerald-500/10 px-2 py-1 font-mono text-[11px] text-emerald-200">{tag}</span>{/each}</div><button type="button" class="mt-2 text-[12px] text-[var(--text-secondary)] hover:text-neutral-200" onclick={onResetContext}>Search globally</button></section>
  {/if}
