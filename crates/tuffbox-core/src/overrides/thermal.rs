...
impl ModOverride for ThermalOverride {
    fn mod_id(&self) -> &str { "thermal" }

    fn config_locations(&self) -> Vec<ModConfigLocation> {
        vec![
            ModConfigLocation { path: "config/thermal/thermal-core.toml".to_string(), format: ConfigFileFormat::Toml, description: "Thermal main settings".to_string() },
        ]
    }
