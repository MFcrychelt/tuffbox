...
pub fn build_crash_prompt(ctx: &CrashAiContext) -> String {
    ...
    p.push_str(
        "You are TuffBox AI, a Minecraft modpack crash analyzer.
...
",
    ...
    p.push_str(
        "Analyze the crash and respond with a JSON object containing:
",
    );
    p.push_str(
        "- human_explanation: A clear explanation in plain English
",
    );
    p.push_str(
...
",
    ...
    p.push_str(
        "- suspected_mods: list of mod IDs most likely responsible
",
    );
    p.push_str(
        "- recommended_actions: array of {action_type, mod_id?, description, risk}
",
    );
    p.push_str(
        "- needs_user_review: true if user should verify before applying
",
    );
    p.push_str(
        "- additional_context: any extra helpful information
",
    );
...
pub fn parse_crash_response(json_str: &str) -> Result<CrashAiResponse, String> {
    let v: serde_json::Value =
        serde_json::from_str(json_str).map_err(|e| format!("Invalid AI response JSON: {}", e))?;

    Ok(CrashAiResponse {
        human_explanation: v
            .get("human_explanation")
            .and_then(|s| s.as_str())
